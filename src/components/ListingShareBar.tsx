...
export function ListingShareBar({
  ...
  const links: { label: string; href: string; icon: React.ReactNode }[] = [
    ...
    { label: "X", href: `https://twitter.com/intent/tweet?url=${e(url)}&text=${e(title)}`, icon: <Badge>𝕏</Badge> },
    { label: "LinkedIn", href: `https://www.linkedin.com/sharing/share-offsite/?url=${e(url)}`, icon: <Badge>in</Badge> },
    { label: "WhatsApp", href: `https://wa.me/?text=${e(`${title} ${url}`)}`, icon: <Badge>✆</Badge> },
    { label: "Email", href: `mailto:?subject=${e(title)}&body=${e(url)}`, icon: <Badge>@</Badge> },
    { label: "Text message", href: `sms:?&body=${e(`${title} ${url}`)}`, icon: <Badge>≡</Badge> },
  ];
